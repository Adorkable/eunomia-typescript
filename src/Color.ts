...
export type ColorGradientSharedSize = 'closest-side' | 'closest-corner' | 'farthest-side' | 'farthest-corner'
...
export const BlackAndWhite: ColorSet = [
  0x000000,
  0xffffff
]
...
export const colorTypeWithAlphaToHexString = (color: Color, alpha: number): string => {
...
export const colorGradientToCSS = (colorGradient: ColorGradient): string => {
  ...
  const colorStops = colorGradient.colorStops.map((colorStop: ColorGradientColorStop): string => {
    return `${colorTypeToHexString(colorStop.color)} ${(colorStop.location * 100).toString(10)}%`
  })
...
export const colorSetToEvenColorGradient = (colorSet: ColorSet): ColorGradient => {
  ...
  const result: ColorGradient = {
    type,
    colorStops: colorSet.map((color: Color, index: number): ColorGradientColorStop => {
      let location: number
      if (index === 0) {
        location = 0.0
