export const orderMinMax = (minimum: number, maximum: number): [number, number] => {
    return [
...
export const orderedMinMaxWrapper = (perform: (number: number, minimum: number, maximum: number) => number): ((number: number, minimum: number, maximum: number) => number) => {
