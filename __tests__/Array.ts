...
test('compareArrayContents', async () => {
    expect(await compareArrayContents(
        [], []
    )).toBeTruthy()
    ...
    expect(await compareArrayContents(
        [0, 1, 3, 234, "aldkfj", 34, undefined], [0, 1, 3, 234, "aldkfj", 34, undefined]
    )).toBeTruthy()

    expect(await compareArrayContents(
        [0, 1, 3, 234, "aldkfj", 34, null], [0, 1, 3, 234, "aldkfj", 34, undefined]
    )).toBeFalsy()

    expect(await compareArrayContents(
        [0, 1, 3, 234, "aldkfj", 34, null], [0, 1, 3, 1, "aldkfj", 34, undefined]
    )).toBeFalsy()

    expect(await compareArrayContents(
        [0, 1, 3, 1, "aldkfj", 34, undefined], [0, 1, 3, 234, "aldkfj", 34, undefined]
    )).toBeFalsy()

    expect(await compareArrayContents(
        [0, 1, 3, 234, "aldkfj", 34, null], [0, 1, 3, 234, "aldkfj", 34]
    )).toBeFalsy()
})

const compareArrayToIndexMap = <ValueType>(array: Array<ValueType>, indexMap: Record<string, ValueType>, mapTo?: (value: any, indexKey: string) => ValueType) => {
